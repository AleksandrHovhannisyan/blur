import { BLUR_INTENSITY_CUSTOM_PROPERTY, BLUR_MENU_ITEM_ID } from "./constants.js";
...
chrome.runtime.onInstalled.addListener((details) => {
  ...
  chrome.contextMenus.onClicked.addListener(async (info, tab) => {
    if (
      ...
      info.menuItemId === BLUR_MENU_ITEM_ID
    ) {
      ...
      chrome.scripting.executeScript({
        ...
        func: (defaultBlurIntensity, BLUR_INTENSITY_CUSTOM_PROPERTY) => {
          ...
          const getMediaDiagonal = (width: number, height: number) =>
          ...
          const blurElement = (element: HTMLElement) => {
            ...
            if (MEDIA_TAGS.has(element.tagName.toLowerCase())) {
              ...
              const diagonalLengthPx = getMediaDiagonal(
                element.clientWidth,
                element.clientHeight
              );
              blurAmount = `${Math.max(
                (diagonalLengthPx / MEDIA_BASE_DIAGONAL_LENGTH_PX) *
                  MEDIA_BASE_BLUR_MULTIPLIER,
                MEDIA_MIN_BLUR_PX
              )}px`;
            } else {
              blurAmount = `0.5em`;
